 ...
 */
public class Evadomatic3000v1 extends AdvancedRobot {
    ...
    private static final int NUM_BINS_DISTANCE = 6;
    ...
    // TODO: try weighing by time to impact instead of using absolutely closest
    private Wave getMostDangerousWave() {
        ...
        for (RobotData enemy : enemies.values()) {
            for (Wave wave : enemy.waves) {
                ...
                if (distance < closestDistance && distance > wave.velocity) { // TODO: adjust?
    ...
    private static int getDistanceBinForWaveHit(Wave wave, Point2D.Double hitAt) {
        double distance = wave.center.distance(hitAt);
        ...
        if (evilWave != null) {
            ...
            for (int distanceI = 0; distanceI < NUM_BINS_DISTANCE; distanceI++) {
                for (int angleI = 0; angleI < NUM_BINS_ANGLE; angleI++) {
                    ...
                    enemy.stats[distanceI][angleI] +=
                            0.5 / (Math.pow(angleBucket - angleI, 2) + 1)
                                    + 0.5 / (Math.pow(distanceBucket - distanceI, 2) + 1);
                }
            }
        }
    ...
    private Point2D.Double predictInterception(Wave wave, int direction) {
        ...
        do {
            ...
            double ninetyDegAngle = calculateBearing(wave.center, result) + (direction * NINETY_DEG_RAD);
    ...
    private double getRiskForDirection(Wave wave, int direction) {
        ...
        return wave.robotData.stats[distanceBucket][angleBucket];
    }
    ...
    private void surf() {
        ...
        double moveAngle = wallSmoothing(
                myPos,
                waveAngle + NINETY_DEG_RAD * betterDirection,
                betterDirection
        );
    ...
    static class Wave {
        ...
        public Wave(RobotData robotData, Point2D.Double center, long startTicks, double velocity, double angle, int direction, double progress) {
    ...
    static class RobotData {
        double[][] stats = new double[NUM_BINS_DISTANCE][NUM_BINS_ANGLE];
        Point2D.Double pos = new Point2D.Double();
        List<Wave> waves = new ArrayList<>();
        List<TimestampedEntry<Integer>> surfDirections = new LinkedList<>();
        List<TimestampedEntry<Double>> surfAngles = new LinkedList<>();
        List<TimestampedEntry<Double>> energyLevels = new LinkedList<>();
        double distance;
        long lastSeen;
        double heading;
